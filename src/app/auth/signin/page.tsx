    ...
    <div className="w-full max-w-sm">
      <div className="text-center mb-8">
        <a href="/" className="inline-block hover:opacity-80 transition-opacity">
          <Image src="/volntir_logo_light.svg" alt="Volntir" width={160} height={42} className="mx-auto" priority />
        </a>
        <p className="text-gray-500 mt-3 text-sm">Sign in to manage your events</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <form onSubmit={handleCredentials} className="space-y-3">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-brand hover:bg-brand-hover text-white text-sm font-semibold py-2.5 px-4 rounded-lg transition-colors disabled:opacity-50 shadow-sm"
          >
      ...
      <p className="text-center text-xs text-gray-400 mt-6">
        ...
        <a href={`/auth/signup?callbackUrl=${encodeURIComponent(callbackUrl)}`} className="text-brand hover:text-brand-hover">
          Sign up
        </a>
        {" · "}
